    ...
    def register(self):
        if self.var_First_name.get()=="" or self.var_last_name.get()=="" or self.var_contact.get()=="" or self.var_email.get()=="" or self.var_pass.get()=="" or self.var_pass_2.get()=="" or self.var_sey_question.get()=="" or self.var_s_a.get()=="":
            ...
            if (len(p)<=8 and p!="")or (len(p1)<=8 and p!=""):
                ...
                con=sqlite3.connect(database="form_db.db")
                c=con.cursor()
                c.execute("Select * from form  where Email=?",(self.var_email.get(),))
                r=c.fetchone()
                c.execute("Select * from form where password=?",(self.var_pass.get(),))
                p=c.fetchone()
                if r!=None:
                    ...
                    try:
                        c.execute("insert into form (F_Name,L_Name,Contact,Email,question,ans ,password)values(?,?,?,?,?,?,?)",(self.var_First_name.get(),self.var_last_name.get(),self.var_contact.get(),self.var_email.get(),self.var_sey_question.get(),self.var_s_a.get(),self.var_pass.get()))
                        con.commit()
                        con.close()
